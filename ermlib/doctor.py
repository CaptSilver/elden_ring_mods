...
def run_doctor(game_dir, report):
    ...
    if state != "absent" and forbidden:
        # start_protected_game.exe still exists, so a vanilla online launch is
        # still possible. A proxy DLL "disarms" EAC's own clean-load check, but
        # it does so by hooking the process via the OS DLL search order BEFORE
        # EAC's hooks activate — so "disarmed" is not safer here, it's how the
        # mod loads unnoticed. Treat armed and disarmed the same: exe present +
        # forbidden artifact present is the dangerous mixed state.
