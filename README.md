...
| `erm apply [profile]` | Installs the locked mod set into `Game/`. Re-injects `COOP_PASSWORD` into `ersc_settings.ini` every time, since an ERSC update blanks it. When the game is a newer build than the ancestor the mods were built against, every regulation.bin merge is folded onto the game's own file so the rows the patch added survive — the param layouts are gated first, and the result is checked after. Idempotent: two applies in a row produce the same files. |
...
| `erm refresh [--dry-run]` | Prints what rebuilding the stack against the installed game build would take, in order — adopt the new regulation as the merge baseline, re-resolve pins, gate the param layouts, rebuild, verify, re-stamp. It only prints; `erm apply` runs every one of those steps except re-resolving pins (that's `erm update`). A build that changed without a patch is refused outright. |
